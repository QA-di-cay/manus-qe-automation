...
import { assertSortedAsc, assertSortedDesc } from '@utils';
import { expect } from '@playwright/test';

const companyName = "Automation Test";
const value = generateRandomString(10);
const category = generateRandomString(10);
const stamp = new Date().toISOString().replace(/[:.]/g, '-');

test.describe('PTB Report testcase - @pTBReport', () => {
    ...
    test('[TC-1293] Contract Number: User can click [Add Contract Number] button to add a new [Contract Number]', async ({ sharedPage }) => {
        const header = new Header(sharedPage);
        const pTBReportPage = await header.accessPtbReport();
    ...
    test('[TC-1294] Contract Number: User can edit Contract Number by clicking on [Edit] icon', async ({ sharedPage }) => {
        ...
        await pTBReportPage.addConTractNumber(value);
        await pTBReportPage.verifyAddConTractNumberSuccess(value);
        await pTBReportPage.editConTractNumber(value);
        await pTBReportPage.verifyEditConTractNumberSuccess(value);
    })
